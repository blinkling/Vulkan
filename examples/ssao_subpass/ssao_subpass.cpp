...
public:
	...
	struct {
		struct Offscreen : public FrameBuffer {
			FrameBufferAttachment position, normal, albedo, depth, ssao, blur;
		} offscreen;
	...
	~VulkanExample()
	{
		...
		frameBuffers.offscreen.ssao.destroy(device);
		frameBuffers.offscreen.blur.destroy(device);
	...
	void prepareOffscreenFramebuffers()
	{
		...
		// SSAO
		createAttachment(VK_FORMAT_R8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.offscreen.ssao, width, height);				// Color
		...
		// SSAO blur
		createAttachment(VK_FORMAT_R8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.offscreen.blur, width, height);					// Color
		...
		for (uint32_t i = 0; i < static_cast<uint32_t>(attachmentDescs.size()); i++)
		{
			...
			attachmentDescs[i].finalLayout = (i == 5) ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		}
		...
		attachmentDescs[3].format = frameBuffers.offscreen.ssao.format;
		attachmentDescs[4].format = frameBuffers.offscreen.blur.format;

		//attachmentDescs[3].format = frameBuffers.ssao.color.format;
		//attachmentDescs[4].format = frameBuffers.ssaoBlur.color.format;


		attachmentDescs[5].format = frameBuffers.offscreen.depth.format;
		...
		//g-buffer
		{
			...
			subpass_gbuffer.pColorAttachments = colorReferences.data();
			subpass_gbuffer.colorAttachmentCount = static_cast<uint32_t>(colorReferences.size());
			subpass_gbuffer.pDepthStencilAttachment = &depthReference;

			subpasses.emplace_back(subpass_gbuffer);
		}
		//ssao
		{
			VkSubpassDescription subpass_ssao = {};

			VkAttachmentReference colorReference = { 3, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL };
			...
			uint32_t preserveAttachment = 2;//2∫≈ «albedo

			VkSubpassDescription subpass_ssao = {};
			subpass_ssao.pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;
			subpass_ssao.pInputAttachments = inputReferences;
			subpass_ssao.inputAttachmentCount = 2;
			subpass_ssao.pColorAttachments = &colorReference;
			subpass_ssao.colorAttachmentCount = 1;
			subpass_ssao.pPreserveAttachments = &preserveAttachment;
			subpass_ssao.preserveAttachmentCount = 1;

			subpasses.emplace_back(subpass_ssao);

		}
		//ssao blur
		{
			VkAttachmentReference colorReference = { 4, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL };
			VkAttachmentReference inputReference = { 3, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
			uint32_t preserveAttachment = 2;//2∫≈ «albedo

			VkSubpassDescription subpass_blur = {};
			subpass_blur.pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;
			subpass_blur.pInputAttachments = &inputReference;
			subpass_blur.inputAttachmentCount = 1;
			subpass_blur.pColorAttachments = &colorReference;
			subpass_blur.colorAttachmentCount = 1;
			subpass_blur.pPreserveAttachments = &preserveAttachment;
			subpass_blur.preserveAttachmentCount = 1;

			subpasses.emplace_back(subpass_blur);
		}
		...
		attachments[3] = frameBuffers.offscreen.ssao.view;
		attachments[4] = frameBuffers.offscreen.blur.view;
		attachments[5] = frameBuffers.offscreen.depth.view;
		...
		//	//color
		//	attachmentDescs[0].format = frameBuffers.ssao.color.format;
		...
		//	//input
		//	attachmentDescs[1].format = frameBuffers.offscreen.position.format;
		...
		//	attachmentDescs[2].format = frameBuffers.offscreen.normal.format;
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			*/
			...
			// First sub pass Fill G-Buffer components (positions+depth, normals, albedo) using MRT
			{
	...
	void setupLayoutsAndDescriptors()
	{
		...
		// SSAO Generation
		setLayoutBindings = {
			vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, VK_SHADER_STAGE_FRAGMENT_BIT, 0),						// FS Position+Depth
			vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, VK_SHADER_STAGE_FRAGMENT_BIT, 1),						// FS Normals
		...
		writeDescriptorSets = {
			vks::initializers::writeDescriptorSet(descriptorSets.ssao, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 0, &imageDescriptors[0]),					// FS Position+Depth
			vks::initializers::writeDescriptorSet(descriptorSets.ssao, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 1, &imageDescriptors[1]),					// FS Normals
		...
		// SSAO Blur
		setLayoutBindings = {
			vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, VK_SHADER_STAGE_FRAGMENT_BIT, 0),						// FS Sampler SSAO
		};
		setLayoutCreateInfo = vks::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()));
		...
		imageDescriptors = {
			vks::initializers::descriptorImageInfo(colorSampler, frameBuffers.offscreen.ssao.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL),
		};
		writeDescriptorSets = {
			vks::initializers::writeDescriptorSet(descriptorSets.ssaoBlur, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 0, &imageDescriptors[0]),
		};
		vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
		...
		imageDescriptors = {
			...
			vks::initializers::descriptorImageInfo(colorSampler, frameBuffers.offscreen.ssao.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL),
			vks::initializers::descriptorImageInfo(colorSampler, frameBuffers.offscreen.blur.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL),
		};
		writeDescriptorSets = {
