/*
* Vulkan Example - Example for VK_EXT_debug_utils extension. To be used in conjuction with a debugging app like RenderDoc (https://renderdoc.org)
*
...
struct Scene {
	...
	void draw(VkCommandBuffer cmdBuffer)
	{
		...
		for (auto i = 0; i < model.parts.size(); i++)
		{
			...
			DebugUtils::insert(cmdBuffer, "Draw \"" + modelPartNames[i] + "\"", glm::vec4(0.0f));
...
public:
	...
	void prepareOffscreen()
	{
		...
		DebugUtils::setObjectName(device, (uint64_t)offscreenPass.color.image, VK_OBJECT_TYPE_IMAGE, "Off-screen color framebuffer");
		DebugUtils::setObjectName(device, (uint64_t)offscreenPass.depth.image, VK_OBJECT_TYPE_IMAGE, "Off-screen depth framebuffer");
		DebugUtils::setObjectName(device, (uint64_t)offscreenPass.sampler, VK_OBJECT_TYPE_SAMPLER, "Off-screen framebuffer default sampler");
	}
	...
	void loadScene()
	{
		...
		// Scene
		DebugUtils::setObjectName(device, (uint64_t)scene.model.vertices.buffer, VK_OBJECT_TYPE_BUFFER, "Scene vertex buffer");
		DebugUtils::setObjectName(device, (uint64_t)scene.model.indices.buffer, VK_OBJECT_TYPE_BUFFER, "Scene index buffer");
		// Glow
		DebugUtils::setObjectName(device, (uint64_t)sceneGlow.model.vertices.buffer, VK_OBJECT_TYPE_BUFFER, "Glow vertex buffer");
		DebugUtils::setObjectName(device, (uint64_t)sceneGlow.model.indices.buffer, VK_OBJECT_TYPE_BUFFER, "Glow index buffer");
	}
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			*/
			if (glow)
			{
				...
				// Start a new debug marker region
				DebugUtils::beginRegion(drawCmdBuffers[i], "Off-screen scene rendering", glm::vec4(1.0f, 0.78f, 0.05f, 1.0f));
				...
				DebugUtils::endRegion(drawCmdBuffers[i]);
			}
			...
			*/
			{
				...
				// Start a new debug marker region
				DebugUtils::beginRegion(drawCmdBuffers[i], "Render scene", glm::vec4(0.5f, 0.76f, 0.34f, 1.0f));
				...
				// Start a new debug marker region
				DebugUtils::beginRegion(drawCmdBuffers[i], "Toon shading draw", glm::vec4(0.78f, 0.74f, 0.9f, 1.0f));
				...
				{
					// Insert debug marker
					DebugUtils::beginRegion(drawCmdBuffers[i], "Wireframe draw", glm::vec4(0.53f, 0.78f, 0.91f, 1.0f));
				...
				if (glow)
				{
					DebugUtils::beginRegion(drawCmdBuffers[i], "Apply post processing", glm::vec4(0.93f, 0.89f, 0.69f, 1.0f));
	...
	void setupDescriptorSetLayout()
	{
		...
		// Name for debugging
		DebugUtils::setObjectName(device, (uint64_t)pipelineLayout, VK_OBJECT_TYPE_PIPELINE_LAYOUT, "Shared pipeline layout");
		DebugUtils::setObjectName(device, (uint64_t)descriptorSetLayout, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, "Shared descriptor set layout");
	}
	...
	void preparePipelines()
	{
		...
		//这里用的和debugmarker的一样,就直接从debugmarker的文件夹拿了
		...
		DebugUtils::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 0], VK_OBJECT_TYPE_SHADER_MODULE, "Toon shading vertex shader");
		DebugUtils::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 1], VK_OBJECT_TYPE_SHADER_MODULE, "Toon shading fragment shader");
		DebugUtils::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 2], VK_OBJECT_TYPE_SHADER_MODULE, "Color-only vertex shader");
		DebugUtils::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 3], VK_OBJECT_TYPE_SHADER_MODULE, "Color-only fragment shader");
		DebugUtils::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 4], VK_OBJECT_TYPE_SHADER_MODULE, "Postprocess vertex shader");
		DebugUtils::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 5], VK_OBJECT_TYPE_SHADER_MODULE, "Postprocess fragment shader");

		// Name pipelines for debugging
		DebugUtils::setObjectName(device, (uint64_t)pipelines.toonshading, VK_OBJECT_TYPE_PIPELINE, "Toon shading pipeline");
		DebugUtils::setObjectName(device, (uint64_t)pipelines.color, VK_OBJECT_TYPE_PIPELINE, "Color only pipeline");

		if (deviceFeatures.fillModeNonSolid)
		{
			DebugUtils::setObjectName(device, (uint64_t)pipelines.wireframe, VK_OBJECT_TYPE_PIPELINE, "Wireframe rendering pipeline");
		}

		DebugUtils::setObjectName(device, (uint64_t)pipelines.postprocess, VK_OBJECT_TYPE_PIPELINE, "Post processing pipeline");
	}
	...
	void prepareUniformBuffers()
	{
		...
		DebugUtils::setObjectName(device, (uint64_t)uniformBuffer.buffer, VK_OBJECT_TYPE_BUFFER, "Scene uniform buffer block");
		// Add some random tag
		DebugUtils::setObjectTag(device, (uint64_t)uniformBuffer.buffer, VK_OBJECT_TYPE_BUFFER, 0, sizeof(demoTag), &demoTag);
	...
	virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay)
	{
		if (overlay->header("Info")) {
			overlay->text("VK_EXT_debug_utils %s", (DebugUtils::active ? "active" : "not present"));
		}
